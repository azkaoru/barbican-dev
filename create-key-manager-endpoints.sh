...
echo "=== Alternative: Localhost endpoints for development ==="
echo "For local development with Barbican running on localhost:9311:"
...
echo ""
echo "NOTE: These endpoints should match where your Barbican API service is actually running."
echo "If Barbican runs on localhost:9311, then these endpoints are correct."
echo "If Barbican runs on a different host/port, update the URLs accordingly."
echo ""
...
echo "2. Barbican (key-manager) service is running on the target host/port"
echo "   - For VIP setup: Barbican running on VIP:9311"
echo "   - For localhost setup: Barbican running on localhost:9311"
echo "   - The endpoint URLs must match where Barbican is actually accessible"
echo "3. Admin credentials are properly configured"
echo "4. The target host/port is accessible from clients that will use the service"
echo ""
